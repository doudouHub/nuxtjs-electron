...
const start = (config, options, nuxt) => {
  ...
  let startOptions = [path.join(config.output.path, 'main.js')];
  if (process.env.NODE_ENV === 'development') {
    startOptions.push('--inspect=' + process.env.INSPECT);
  }
  instance = spawn(electron, startOptions, {env: process.env});
...
module.exports = async function nuxtElectron (options = {}) {
  ...
  this.extendBuild((config, {isClient, isDev}) => {
    ...
    } // https://github.com/nuxt/nuxt.js/issues/2892
  });
};
